...
- 听力练习：当前仅展示入口，详情页暂未实现；点击后提示“听力练习暂未开放”。

前三个已实现入口在未登录时仍统一跳转登录页。听力练习不注册新页面、不调用后端接口，后续实现详情页时再补充 `app.json` 页面路由与业务接口说明。
...
- 底部输入栏默认是语音输入，一个输入区域内通过左侧系统风格图标在语音和文字输入间切换；语音录制结束后自动切到文字输入，方便用户编辑识别文本后发送。
...
- 单词本列表和学习首页使用轻量白色卡片、扁平分段控件、低饱和进度信息块。
