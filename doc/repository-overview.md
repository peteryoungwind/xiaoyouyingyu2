...
- 登录用户可从微信小程序首页或学习中心进入每日外刊列表。
- 小程序用户端列表支持未读/已读筛选，详情页进入后自动记录该用户已读。
- 详情页展示音频、英文段落、可切换的逐段中文翻译、文章总结、重点词汇和表达句型。
- 管理员可在 `/admin/daily-articles` 维护外刊库存、段落、音频 URL 或上传音频文件，并手动生成今日外刊。
- 后端每天 `Asia/Shanghai` 早上 6 点从未推送且启用的外刊中随机发布一篇。
...
| 登录用户 | 搜索话题、修改账号信息、查看会员状态、兑换卡密、阅读每日外刊 |
| 会员用户 | 进入学习中心，调用 AI 学习内容接口 |
| 管理员 | 话题管理、每日外刊管理、用户管理、AI 模型管理、卡密管理、会员管理 |
